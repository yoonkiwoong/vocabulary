...
def record_batch(entries: list) -> None:
    require_fsrs()
    if not DB_PATH.exists():
        raise FileNotFoundError(f"database not found: {DB_PATH}")

    reviewed_at_iso = utc_now().isoformat()
    scheduler = Scheduler()
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for entry in entries:
            ...
            if RATING_MAP.get(rating_str) is None:
                raise ValueError(f"invalid rating '{rating_str}' for word_id={word_id}")
    ...
    print(f"Batch complete: {len(entries)} words recorded.")


if __name__ == "__main__":
    entries = json.load(sys.stdin)
    try:
        record_batch(entries)
